...
export class EntityCardDataImpl implements EntityData {
    constructor(public name: string, public type: string, public target: string, public params: Record<string, unknown>) { }
        ...
        if (key.endsWith('.proxy_featured') && value === true) {
            ...
            if (target && paramsStr) {
                try {
                    ...
                    featured.push(new EntityCardDataImpl(target, params.type || 'unknown', target, params));
                }
                catch (parseError) {
    ...
    for (const [key, value] of Object.entries(allParams)) {
        if (key.endsWith('.featured') && value === true) {
            ...
            if (paramsStr) {
                try {
                    const params = JSON.parse(paramsStr);
                    featured.push(new EntityCardDataImpl(displayName, params.type || 'unknown', displayName, params));
                }
                catch (parseError) {
                    logger.warn(`Failed to parse params for ${nodeName}:`, parseError);
                }
                ...
                featured.push(new EntityCardDataImpl(displayName, 'unknown', displayName, {}));
            }
        }
    }
    return featured;
