...
import type { SensorMsgsNavSatFix, StdMsgsFloat64, MavrosMsgsState, MavrosMsgsExtendedState, SensorMsgsBatteryState, GeometryMsgsPoseStamped, GeometryMsgsTwistStamped, SensorMsgsImu, MavrosMsgsAltitude, MavrosMsgsHomePosition, } from '../ros/ros-types.js';
...
type RosFrame = RosPublish<SensorMsgsNavSatFix | StdMsgsFloat64 | MavrosMsgsState | MavrosMsgsExtendedState | SensorMsgsBatteryState | GeometryMsgsPoseStamped | GeometryMsgsTwistStamped | SensorMsgsImu | MavrosMsgsAltitude | MavrosMsgsHomePosition> | Record<string, unknown>;
type EventMap = {
    update: (state: Partial<DroneState>) => void;
    statusUpdate: (state: Partial<DroneState>) => void;
};
...
abstract class EventEmitter extends EntityStateModel {
    ...
    emit<K extends keyof EventMap>(event: K, ...args: Parameters<EventMap[K]>) {
        this.eventListeners.get(event)?.forEach(fn => { try {
            (fn as any)(...args);
        }
        catch { } });
    }
...
export class DroneStateModel extends EventEmitter {
    ...
    public onSectionChange<K extends keyof DroneState>(section: K, listener: SectionChangeListener<Partial<DroneState>[K]>) {
        if (!this.sectionChangeListeners.has(section))
            this.sectionChangeListeners.set(section, new Set());
    ...
    private handleGlobalFix(msg: any) {
        ...
        if (t !== null)
            this.state.global_position_int!.time_boot_ms = t;
    ...
    private ensureExtended() { if (!this.state.extended)
        this.state.extended = { time_boot_ms: Date.now() }; }
    private ensureBattery() { if (!this.state.battery)
        this.state.battery = { time_boot_ms: Date.now(), temperature: null }; }
    private ensureLocal() {
        if (!this.state.local) {
            this.state.local = {
                time_boot_ms: Date.now(),
    ...
    private ensureAltitude() { if (!this.state.altitude)
        this.state.altitude = { time_boot_ms: Date.now() }; }
    private ensureHome() { if (!this.state.home)
        this.state.home = { time_boot_ms: Date.now(), lat: 0, lon: 0, alt: 0 }; }
    ...
    private omitTime(obj: any): any {
        ...
        if (Array.isArray(obj))
            return obj.map(item => this.omitTime(item));
        const result: any = {};
    ...
    private updateRotationFromHeading() {
        ...
        if (this.state.attitude)
            this.state.attitude.yaw = hdgRad;
    }
    ...
    private refreshStatus() {
        ...
        if (landedEff === undefined || landedEff === LANDED.UNDEFINED) {
            ...
            if (!armed && almostOnGround)
                landedEff = LANDED.ON_GROUND;
        }
        const faults: string[] = [];
        if (!gcs)
            faults.push('vehicle.link.down');
        if (!seen(this.T_IMU))
            faults.push('imu.stale');
        if (!seen(this.T_FIX))
            faults.push('gps.stale');
        const pct = this.state.battery?.percentage;
        const v = this.state.battery?.voltage;
        if (typeof pct === 'number' && pct <= 0.15)
            faults.push('battery.low');
        if (typeof v === 'number' && v > 0 && v < 10.5)
            faults.push('battery.voltage.low');
        const armedNow = !!this.state.vehicle?.armed;
        if (!armedNow && landedEff === LANDED.IN_AIR)
            faults.push('state.inconsistent');
        let armable = true;
